...
#ifndef __INET_RQALGORITHM_H
#define __INET_RQALGORITHM_H
...
/**
 * Abstract base class for RQ algorithms which encapsulate all behaviour
 ...
 */
class INET_API RQAlgorithm : public cObject
{
  protected:
    RQConnection *conn;    // we belong to this connection
    RQStateVariables *state;    // our state variables

    /**
     * Create state block (TCB) used by this RQ variant. It is expected
     * that every RQAlgorithm subclass will have its own state block,
     * subclassed from RQStateVariables. This factory method should
     * create and return a "blank" state block of the appropriate type.
     */
    virtual RQStateVariables *createStateVariables() = 0;

  public:
    /**
     * Ctor.
     */
    RQAlgorithm() { state = nullptr; conn = nullptr; }
    ...
    /**
     * Assign this object to a RQConnection. Its sendQueue and receiveQueue
     * must be set already at this time, because we cache their pointers here.
     */
    void setConnection(RQConnection *_conn) { conn = _conn; }

    /**
     * Creates and returns the RQ state variables.
     */
    RQStateVariables *getStateVariables()
    {
        if (!state)
     ...
     * This method is necessary because the RQConnection ptr is not
    ...
    /**
     * Place to process timers specific to this RQAlgorithm class.
     * RQConnection will invoke this method on any timer (self-message)
     * it doesn't recognize (that is, any timer other than the 2MSL,
     * CONN-ESTAB and FIN-WAIT-2 timers).
     *
     * Method may also change the event code (by default set to RQ_E_IGNORE)
     * to cause the state transition of RQ FSM.
     */
    virtual void processTimer(cMessage *timer, RQEventCode& event) = 0;

    /**
     * Called after user sent RQ_C_SEND command to us.
     */
...
#endif // ifndef __INET_RQALGORITHM_H
