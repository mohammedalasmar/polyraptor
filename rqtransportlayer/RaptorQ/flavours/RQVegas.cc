...
Register_Class(RQVegas);

RQVegasStateVariables::RQVegasStateVariables()
{
    ssthresh = 65535;
    v_recoverypoint = 0;
    v_cwnd_changed = 0;
...
std::string RQVegasStateVariables::info() const
{
...
std::string RQVegasStateVariables::detailedInfo() const
{
    std::stringstream out;
...
RQVegas::RQVegas()
    : RQBaseAlg(), state((RQVegasStateVariables *&)RQAlgorithm::state)
{
}

// Same as TCPReno
void RQVegas::recalculateSlowStartThreshold()
{
    // RFC 2581, page 4:
...
void RQVegas::processRexmitTimer(RQEventCode& event)
{
    RQBaseAlg::processRexmitTimer(event);
...
void RQVegas::receivedDataAck(uint32 firstSeqAcked)
{
    RQBaseAlg::receivedDataAck(firstSeqAcked);

    const RQSegmentTransmitInfoList::Item *found = state->regions.get(firstSeqAcked);
    if (found) {
        ...
        if (state->v_worried > 0) {
            ...
            const RQSegmentTransmitInfoList::Item *unaFound = state->regions.get(state->snd_una);
            ...
            // RQConnection::retransmitOneSegment will fail: ASSERT(bytes!=0), line 839), because bytes = snd_max-snd_una
...
void RQVegas::receivedDuplicateAck()
{
    ...
    simtime_t tSent = 0;
    int num_transmits = 0;
    const RQSegmentTransmitInfoList::Item *found = state->regions.get(state->snd_una);
    if (found) {
...
void RQVegas::dataSent(uint32 fromseq)
{
    RQBaseAlg::dataSent(fromseq);
...
void RQVegas::segmentRetransmitted(uint32 fromseq, uint32 toseq)
{
    RQBaseAlg::segmentRetransmitted(fromseq, toseq);
