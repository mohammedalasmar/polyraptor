...
Register_Class(RQReno);

RQReno::RQReno() : RQTahoeRenoFamily(),
    state((RQRenoStateVariables *&)RQAlgorithm::state)
{
}

void RQReno::recalculateSlowStartThreshold()
{
    // RFC 2581, page 4:
    // "When a RQ sender detects segment loss using the retransmission
...
void RQReno::processRexmitTimer(RQEventCode& event)
{
    RQTahoeRenoFamily::processRexmitTimer(event);

    if (event == RQ_E_ABORT)
        return;

    // After REXMIT timeout RQ Reno should start slow start with snd_cwnd = snd_mss.
    //
    ...
    // the RQ sender uses the slow start algorithm to increase the window
...
void RQReno::receivedDataAck(uint32 firstSeqAcked)
{
    RQTahoeRenoFamily::receivedDataAck(firstSeqAcked);
    ...
    if (state->dupacks >= DUPTHRESH) {    // DUPTHRESH = 3
        ...
        if (state->snd_cwnd < state->ssthresh) {
            ...
            // perform Slow Start. RFC 2581: "During slow start, a RQ increments cwnd
            ...
            // implemented in real-life RQs, hence it's commented out. Also, the ABC
    ...
    if (state->sack_enabled && state->lossRecovery) {
        // RFC 3517, page 7: "Once a RQ is in the loss recovery phase the following procedure MUST
    ...
    // contain SACK information.  In this case, a RQ sender SHOULD use this
...
void RQReno::receivedDuplicateAck()
{
    RQTahoeRenoFamily::receivedDuplicateAck();

    if (state->dupacks == DUPTHRESH) {    // DUPTHRESH = 3
        EV_INFO << "Reno on dupAcks == DUPTHRESH(=3): perform Fast Retransmit, and enter Fast Recovery:";

        if (state->sack_enabled) {
            // RFC 3517, page 6: "When a RQ sender receives the duplicate ACK corresponding to
            ...
            //
            // When the RQ sender receives a cumulative ACK for this data octet
        ...
        // (...) the RQ sender can continue to transmit new
...
//            // the RQ sender but for which no cumulative or selective
...
//        }
        ...
        // Note: Steps (A) - (C) of RFC 3517, page 7 ("Once a RQ is in the loss recovery phase the following procedure MUST be used for each arriving ACK")
