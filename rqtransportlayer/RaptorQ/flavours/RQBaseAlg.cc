...
//
// RFC 1122, page 95:
// "A RQ SHOULD implement a delayed ACK, but an ACK should not
...
RQBaseAlgStateVariables::RQBaseAlgStateVariables() {
    rexmit_count = 0;
    rexmit_timeout = 3.0;  // it was 3.0  (moh) ,, if it's 3.0 then ... check
...
std::string RQBaseAlgStateVariables::info() const {
    std::stringstream out;
    out << RQStateVariables::info();
    out << " snd_cwnd=" << snd_cwnd;
    out << " rto=" << rexmit_timeout;
    return out.str();
}

std::string RQBaseAlgStateVariables::detailedInfo() const {
    std::stringstream out;
    out << RQStateVariables::detailedInfo();
    out << "snd_cwnd=" << snd_cwnd << "\n";
    out << "rto=" << rexmit_timeout << "\n";
...
RQBaseAlg::RQBaseAlg() :
        RQAlgorithm(), state((RQBaseAlgStateVariables *&) RQAlgorithm::state) {
    rexmitTimer = persistTimer = delayedAckTimer = keepAliveTimer = nullptr;
    cwndVector = ssthreshVector = rttVector = srttVector = rttvarVector =
            rtoVector = numRtosVector = nullptr;
}

RQBaseAlg::~RQBaseAlg() {
    // Note: don't delete "state" here, it'll be deleted from RQConnection
...
////////////////////////////////////  Step (9)aaa ///////////////////////////////////////
void RQBaseAlg::initialize() {
//    std::cout
//            << "  |||||| STEP (9)aaa ||||||   RQConnection::initConnection ==> rqAlgorithm->initialize(); ==> [[[ RQBaseAlg::initialize() ]]]  "
//            << std::endl;
//    std::cout
...
//            << getFullPath() << std::endl;
    ...
    conn->getRQMain()->appGateIndexTimeOutMap[conn->appGateIndex] = state->numPullsTimeOuts; // moh added
    ...
    if (conn->getRQMain()->recordStatistics) {
...
void RQBaseAlg::established(bool active) {
    ...
    // (MSS) of 1460 bytes (assuming no IP or RQ options).  Therefore,
    ...
    //     then win <= 2 * MSS;
    //
    // This increased initial window is optional: a RQ MAY start with a
    // larger initial window.  However, we expect that most general-purpose
    // RQ implementations would choose to use the larger initial congestion
    ...
    if (state->increased_IW_enabled && state->syn_rexmit_count == 0) {
        state->snd_cwnd = std::min(4 * state->snd_mss,
                std::max(2 * state->snd_mss, (uint32) 4380));
        EV_DETAIL << "Enabled Increased Initial Window, CWND is set to "
                         << state->snd_cwnd << "\n";
    }
    // RFC 2001, page 3:
    ...
    //    if (!sendData(false)) {// FIXME TODO - This condition is never true because the buffer is empty (at this time) therefore the first ACK is never piggyback on data
            // conn->sendAck();    // MOH: HAS BEEN REMOVED
            conn->sendFirstAck();  // MOH: HAS BEEN ADDED
           // std::cout << " sendFirstAck() \n  " ;
 //   }
    }
}

void RQBaseAlg::connectionClosed() {
...
void RQBaseAlg::processTimer(cMessage *timer, RQEventCode& event) {
//    std::cout << " processTimer----------------- " << std::endl;

    if (timer == rexmitTimer) {
//        std::cout << " processTimer--REXMIT-MOH " << std::endl;
...
void RQBaseAlg::processRexmitTimer(RQEventCode& event) {
    ...
    // in RFC 793 above, we'll leave it to subclasses (e.g. RQTahoe, RQReno).
    //
    if (++state->rexmit_count > MAX_REXMIT_COUNT) {
        EV_DETAIL << "Retransmission count exceeds " << MAX_REXMIT_COUNT
                         << ", aborting connection\n";
        conn->signalConnectionTimeout();
        event = RQ_E_ABORT;  // TBD maybe rather introduce a RQ_E_TIMEDOUT event
        return;
    }
    ...
    EV_INFO << " to " << state->rexmit_timeout
                   << "s, and cancelling RTT measurement\n";
...
void RQBaseAlg::processPersistTimer(RQEventCode& event) {
    ...
    // as presented in [Stevens, W.R.: RQ/IP Illustrated, Volume 1, chapter 22.2]
    ...
    state->persist_timeout = state->persist_factor * 1.5; // 1.5 is a factor for typical LAN connection [Stevens, W.R.: RQ/IP Ill. Vol. 1, chapter 22.2]
...
void RQBaseAlg::processDelayedAckTimer(RQEventCode& event) {
    state->ack_now = true;
    conn->sendAck();
}

void RQBaseAlg::processKeepAliveTimer(RQEventCode& event) {
    // TBD
    // RFC 1122, page 102:
    // "A "keep-alive" mechanism periodically probes the other
    // end of a connection when the connection is otherwise
    // idle, even when there is no data to be sent.  The RQ
...
void RQBaseAlg::startRexmitTimer() {
//    std::cout << " aaaaaaaaaaaaaaaaaa start RTO timer      aaaaaaaaaaaaaaaaaa " << state->rexmit_count << std::endl;
...
void RQBaseAlg::rttMeasurementComplete(simtime_t tSent, simtime_t tAcked) {

    // added
    state->rexmit_timeout = 2;
...
//    //
    ...
    // 500ms ticks is available from old RQmodule.cc:calcRetransTimer().
    //
    ...
    // record statistics
    EV_DETAIL << "Measured RTT=" << (newRTT * 1000) << "ms, updated SRTT="
                     << (srtt * 1000) << "ms, new RTO=" << (rto * 1000)
                     << "ms\n";
...
void RQBaseAlg::rttMeasurementCompleteUsingTS(uint32 echoedTS) {
...
bool RQBaseAlg::sendData(bool sendCommandInvoked) {
    //
    // Nagle's algorithm: when a RQ connection has outstanding data that has not
    ...
    // by RQ and sent in a single segment.)
    //
...
//
//    // RFC 2581, pages 7 and 8: "When RQ has not received a segment for
...
//    // (...)
//    // Therefore, a RQ SHOULD set cwnd to no more than RW before beginning
//    // transmission if the RQ has not sent data in an interval exceeding
...
//                        std::min(4 * state->snd_mss,
//                                std::max(2 * state->snd_mss, (uint32) 4380)),
//                        state->snd_cwnd);
//            else
//                state->snd_cwnd = state->snd_mss;
//
//            EV_INFO << "Restarting idle connection, CWND is set to "
//                           << state->snd_cwnd << "\n";
//        }
//    }
    ...
    //
    return conn->sendData(fullSegmentsOnly, state->snd_cwnd ,  isReader ,  isWriter  , opcode ); // Added MOH
}

void RQBaseAlg::sendCommandInvoked(cMessage *msg) {

    // MOH Added
    GenericAppMsgRq *appmsgg = dynamic_cast<GenericAppMsgRq *>(msg);
//    isClient = appmsgg->getIsReader();

    isReader = appmsgg->getIsReader();
    isWriter = appmsgg->getIsWriter();


//    numSymbolsToSend=appmsgg->getNumSymbolsToSend();
    opcode = appmsgg->getOpcode();
//    std::cout << "  RQBaseAlg::sendCommandInvoked,,  isReader " << isReader << " , isWriter "  << isWriter << std::endl ;
...
void RQBaseAlg::receivedOutOfOrderSegment() {
    state->ack_now = true;
    EV_INFO << "Out-of-order segment, sending immediate ACK\n";
    conn->sendAck();
}

void RQBaseAlg::receiveSeqChanged() {
    // If we send a data segment already (with the updated seqNo) there is no need to send an additional ACK
    if (state->full_sized_segment_counter == 0 && !state->ack_now
            && state->last_ack_sent == state->rcv_nxt
            && !delayedAckTimer->isScheduled()) {    // ackSent?
            // RQEV << "ACK has already been sent (possibly piggybacked on data)\n";
    } else {
        // RFC 2581, page 6:
        // "3.2 Fast Retransmit/Fast Recovery
        // (...)
        // In addition, a RQ receiver SHOULD send an immediate ACK
        // when the incoming segment fills in all or part of a gap in the
        // sequence space."
        if (state->lossRecovery)
            state->ack_now = true; // although not mentioned in [Stevens, W.R.: RQ/IP Illustrated, Volume 2, page 861] seems like we have to set ack_now

        if (!state->delayed_acks_enabled) {    // delayed ACK disabled
            EV_INFO << "rcv_nxt changed to " << state->rcv_nxt
                           << ", (delayed ACK disabled) sending ACK now\n";

            //  conn->sendAck(); //MOH: we don't want to ack the data or request packets,,, just ack the FIN packet
            if (state->fin_rcvd == true) {
                conn->sendAck();
            } // added MOH
            ...
            if (state->ack_now) {
                EV_INFO << "rcv_nxt changed to " << state->rcv_nxt
                               << ", (delayed ACK enabled, but ack_now is set) sending ACK now\n";
                conn->sendAck();
            }
            // RFC 1122, page 96: "in a stream of full-sized segments there SHOULD be an ACK for at least every second segment."
                ...
                EV_INFO << "rcv_nxt changed to " << state->rcv_nxt
                               << ", (delayed ACK enabled, but full_sized_segment_counter="
                               << state->full_sized_segment_counter
                               << ") sending ACK now\n";
                conn->sendAck();
            } else {
                EV_INFO << "rcv_nxt changed to " << state->rcv_nxt
                               << ", (delayed ACK enabled and full_sized_segment_counter="
                               << state->full_sized_segment_counter
                               << ") scheduling ACK\n";
...
void RQBaseAlg::receivedDataAck(uint32 firstSeqAcked) {
    ...
    if (state->snd_una == state->snd_max) {
        if (rexmitTimer->isScheduled()) {
            ...
            EV_INFO
                           << "There were no outstanding segments, nothing new in this ACK.\n";
    } else {
    ...
    if (state->snd_wnd == 0) {    // received zero-sized window?
        if (rexmitTimer->isScheduled()) {
            if (persistTimer->isScheduled()) {
                EV_INFO
                               << "Received zero-sized window and REXMIT timer is running therefore PERSIST timer is canceled.\n";
                cancelEvent(persistTimer);
                state->persist_factor = 0;
            } else
                EV_INFO
                               << "Received zero-sized window and REXMIT timer is running therefore PERSIST timer is not started.\n";
        } else {
            if (!persistTimer->isScheduled()) {
                EV_INFO
                ...
                EV_INFO
                               << "Received zero-sized window and PERSIST timer is already running.\n";
        }
    } else {    // received non zero-sized window?
        if (persistTimer->isScheduled()) {
            EV_INFO
                           << "Received non zero-sized window therefore PERSIST timer is canceled.\n";
            cancelEvent(persistTimer);
            state->persist_factor = 0;
        }
    }
...
void RQBaseAlg::receivedDuplicateAck() {
    EV_INFO << "Duplicate ACK #" << state->dupacks << "\n";

    bool fullSegmentsOnly = state->nagle_enabled
    ...
    //
    // Leave to subclasses (e.g. RQTahoe, RQReno) whatever they want to do
    // on duplicate Acks.
    //
    // That is, subclasses will redefine this method, call us, then perform
...
void RQBaseAlg::receivedAckForDataNotYetSent(uint32 seq) {
    // Note: In this case no immediate ACK will be send because not mentioned
    // in [Stevens, W.R.: RQ/IP Illustrated, Volume 2, page 861].
    // To force immediate ACK use:
    // state->ack_now = true;
    // RQEV << "ACK acks something not yet sent, sending immediate ACK\n";
    EV_INFO << "ACK acks something not yet sent, sending ACK\n";
    conn->sendAck();
}

void RQBaseAlg::ackSent() {
...
void RQBaseAlg::dataSent(uint32 fromseq) {
...
void RQBaseAlg::segmentRetransmitted(uint32 fromseq, uint32 toseq) {
}

void RQBaseAlg::restartRexmitTimer() {
    if (rexmitTimer->isScheduled())
        cancelEvent(rexmitTimer);
...
// MOH Added
void RQBaseAlg::cancelRexmitTimer() {
    if (rexmitTimer->isScheduled())
        cancelEvent(rexmitTimer);
}
