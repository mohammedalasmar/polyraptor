...
Register_Class(RQSymbolsRcvQueue);

RQSymbolsRcvQueue::RQSymbolsRcvQueue() : RQVirtualDataRcvQueue()
{
}

RQSymbolsRcvQueue::~RQSymbolsRcvQueue()
{
...
void RQSymbolsRcvQueue::init(uint32 startSeq)
{
    RQVirtualDataRcvQueue::init(startSeq);
}

std::string RQSymbolsRcvQueue::info() const
{
    std::stringstream os;
...
uint32 RQSymbolsRcvQueue::insertBytesFromSegment(RQSegment *rqseg)
{
...
cPacket *RQSymbolsRcvQueue::extractBytesUpTo(uint32 seq)
{
    cPacket *msg = nullptr;
