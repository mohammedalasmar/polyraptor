...
#ifndef __INET_RQSENDQUEUE_H
#define __INET_RQSENDQUEUE_H
...
/**
 * Abstract base class for RQ send queues. In fact a single object
 * represents both the send queue and the retransmission queue
 * (no need to separate them). The RQConnection object knows
 ...
 * it stores bytes and not segments. RQ is a bytestream oriented protocol
 ...
 * "IMPLEMENTATION" note).
 *
 * To simulate a RQ that retains segment boundaries in retransmissions,
 * the appropriate RQAlgorithm class should remember where the segment
 ...
 * You might want to:
 *
 * - transmit a real bytes, especially if the application which uses RQ
 *   is a ported version of a real socket application.
 *
 * - simulate a "dummy" connection, that is, simulated RQ segments
 ...
 *   Here every cMessage maps to a sequence number range in the RQ
 ...
 *   connection.
 *
 * Different RQSendQueue subclasses can be written to accomodate
 * different needs.
 *
 * This class goes hand-in-hand with RQReceiveQueue.
 *
 * @see RQReceiveQueue
 */
class INET_API RQSendQueue : public cObject
{
  protected:
  ...
  public:
    ...
    /**
     * Called when the RQ wants to send or retransmit data, it constructs
     * a RQ segment which contains the data from the requested sequence
     ...
     */
    virtual RQSegment *createSegmentWithBytes(uint32 fromSeq, ulong maxNumBytes) = 0;
...
#endif // ifndef __INET_RQSENDQUEUE_H
