#ifndef __INET_RQCONNECTION_H
#define __INET_RQCONNECTION_H
...
// TIME-WAIT - both FIN's acked, waiting for some time to be sure remote RQ received our ACK
...
enum RQState {
    RQ_S_INIT = 0,
    RQ_S_CLOSED = FSM_Steady(1),
    RQ_S_LISTEN = FSM_Steady(2),
    RQ_S_SYN_SENT = FSM_Steady(3),
    RQ_S_SYN_RCVD = FSM_Steady(4),
    RQ_S_ESTABLISHED = FSM_Steady(5),
    RQ_S_CLOSE_WAIT = FSM_Steady(6),
    RQ_S_LAST_ACK = FSM_Steady(7),
    RQ_S_FIN_WAIT_1 = FSM_Steady(8),
    RQ_S_FIN_WAIT_2 = FSM_Steady(9),
    RQ_S_CLOSING = FSM_Steady(10),
    RQ_S_TIME_WAIT = FSM_Steady(11)
};
...
enum RQEventCode {
    ...
    RQ_E_RCV_FIN,
    RQ_E_RCV_FIN_ACK,
    RQ_E_RCV_RST,    // covers RST+ACK too

    RQ_E_RCV_UNEXP_SYN,    // unexpected SYN

    // timers
    RQ_E_TIMEOUT_2MSL,    // RFC 793, a.k.a. TIME-WAIT timer
    RQ_E_TIMEOUT_CONN_ESTAB,
    RQ_E_TIMEOUT_FIN_WAIT_2,

// All other timers (REXMT, PERSIST, DELAYED-ACK, KEEP-ALIVE, etc.),
// are handled in RQAlgorithm.
};

/** @name Timeout values */
//@{
#define RQ_TIMEOUT_CONN_ESTAB        75  // 75 seconds
#define RQ_TIMEOUT_FIN_WAIT_2        600  // 10 minutes
#define RQ_TIMEOUT_2MSL              240  // 2 * 2 minutes
#define RQ_TIMEOUT_SYN_REXMIT        3  // initially 3 seconds

#define RQ_TIMEOUT_PULL_REXMIT        0.5  // initially 500 msec  MOH

#define RQ_TIMEOUT_SYN_REXMIT_MAX    240  // 4 mins (will only be used with SYN+ACK: with SYN CONN_ESTAB occurs sooner)
//@}

#define MAX_SYN_REXMIT_COUNT          12  // will only be used with SYN+ACK: with SYN CONN_ESTAB occurs sooner
#define RQ_MAX_WIN                   65535  // 65535 bytes, largest value (16 bit) for (unscaled) window size
 #define DUPTHRESH                     3  // used for RQTahoe, RQReno and SACK (RFC 3517)
//#define MAX_SACK_BLOCKS               60  // will only be used with SACK
#define RQ_OPTIONS_MAX_SIZE          40  // 40 bytes, 15 * 4 bytes (15 is the largest number in 4 bits length data offset field), RQ_MAX_HEADER_OCTETS - RQ_HEADER_OCTETS = 40
//#define RQ_OPTION_SACK_MIN_SIZE      10  // 10 bytes, option length = 8 * n + 2 bytes (NOP)
#define RQ_OPTION_TS_SIZE            12  // 12 bytes, option length = 10 bytes + 2 bytes (NOP)
#define PAWS_IDLE_TIME_THRESH         (24 * 24 * 3600)  // 24 days in seconds (RFC 1323)

//typedef std::list<Sack> SackList;

/**
 * Contains state variables ("TCB") for RQ.
 *
 * RQStateVariables is effectively a "struct" -- it only contains
 * public data members. (Only declared as a class so that we can use
 * cObject as base class and make it possible to inspect
 * it in Tkenv.)
 *
 * RQStateVariables only contains variables needed to implement
 * the "base" (RFC 793) RQ. More advanced RQ variants are encapsulated
 * into RQAlgorithm subclasses which can have their own state blocks,
 * subclassed from RQStateVariables. See RQAlgorithm::createStateVariables().
 */
class INET_API RQStateVariables: public cObject {
public:
    RQStateVariables();
...
//    };
    uint32 esi;   // encoding symbol id (24-bit)
    uint8_t sbn;  // source block number (8-bit unsigned integer)
    uint32 request_id;
//    uint32 numSymbolsToGet;
    ...
    uint32 initialSentSymbols; // not used now
    ...
    uint32 numberReceivedSymbols;
    uint32 numberSentSymbols;
    ...
    // because normal rexmit belongs to RQAlgorithm)
    ...
    // RQ_E_RCV_FIN or RQ_E_RCV_FIN_ACK).
    bool fin_ack_rcvd;
    ...
    bool ack_now; // send ACK immediately, needed if delayed_acks_enabled is set
                  // Based on [Stevens, W.R.: RQ/IP Illustrated, Volume 2, page 861].
...
//    SackList sacks_array;    // MAX_SACK_BLOCKS is set to 60
    ...
    // those counters would logically belong to RQAlgorithm, but it's a lot easier to manage them here
...
//    uint32 rcv_sacks;    // number of received sacks
    ...
    uint32 maxRcvBuffer;    // maximal amount of bytes in RQ receive queue
    uint32 usedRcvBuffer;    // current amount of used bytes in RQ receive queue
    uint32 freeRcvBuffer;    // current amount of free bytes in RQ receive queue
    uint32 rqRcvQueueDrops;    // number of drops in RQ receive queue
};

/**
 * Manages a RQ connection. This class itself implements the RQ state
 ...
 * associated with RQ state changes.
 *
 ...
 * to understand.
 *
 * RQConnection objects are not used alone -- they are instantiated and managed
 * by a RQ module.
 *
 * RQConnection "outsources" several tasks to objects subclassed from
 * RQSendQueue, RQReceiveQueue and RQAlgorithm; see overview of this
 * with RQ documentation.
 *
 * Connection variables (TCB) are kept in RQStateVariables. RQAlgorithm
 * implementations can extend RQStateVariables to add their own stuff
 * (see RQAlgorithm::createStateVariables() factory method.)
 *
 * The "entry points" of RQConnnection from RQ are:
 *  - processTimer(cMessage *msg): handle self-messages which belong to the connection
 *  - processRQSegment(RQSegment *RQSeg, Address srcAddr, Address destAddr):
 *    handle segment arrivals
 *  - processAppCommand(cMessage *msg): process commands which arrive from the
 *    application (RQ_C_xxx)
 *
 ...
 *     etc., and processRQSegment() dispatches to processSegmentInListen(),
 *     processSegmentInSynSent() or processSegment1stThru8th().
 *     Those methods will do the REAL JOB.
 *  -# after they return, we'll know the state machine event (RQEventCode,
 *     RQ_E_xxx) for sure, so we can:
 *  -# invoke performStateTransition() which executes the necessary state
 *     transition (for example, RQ_E_RCV_SYN will take the state machine
 *     from RQ_S_LISTEN to RQ_S_SYN_RCVD). No other actions are taken
 *     in this step.
 *  -# if there was a state change (for example, we entered the
 *     RQ_S_ESTABLISHED state), performStateTransition() invokes stateEntered(),
 *     which performs some necessary housekeeping (cancel the CONN-ESTAB timer).
 *
 * When the CLOSED state is reached, RQ will delete the RQConnection object.
 *
 */
class INET_API RQConnection {
public:
...
protected:
    RaptorQ *rqMain = nullptr;    // RQ module

    // RQ state machine
    cFSM fsm;

    // variables associated with RQ state
    RQStateVariables *state = nullptr;

    // RQ queues
    RQSendQueue *sendQueue = nullptr;
    RQReceiveQueue *receiveQueue = nullptr;
    RQDataTransferMode transferMode = RQ_TRANSFER_UNDEFINED; // RQ transfer mode: bytecount, object, bytestream

public:
    // RQSACKRexmitQueue *rexmitQueue = nullptr;
    virtual int getNumRcvdSymbols();
    virtual bool  isConnFinished();
    virtual void  setConnFinished() ;

protected:
     cQueue pullQueue;
    // RQ behavior in data transfer state
    RQAlgorithm *rqAlgorithm = nullptr;
...
//    cOutVector *rcvSacksVector = nullptr;    // number of received Sacks
    ...
    cOutVector *rqRcvQueueBytesVector = nullptr; // current amount of used bytes in RQ receive queue
    cOutVector *rqRcvQueueDropsVector = nullptr; // number of drops in RQ receive queue

protected:
    /** @name FSM transitions: analysing events and executing state transitions */
    //@{
    /** Maps app command codes (msg kind of app command msgs) to RQ_E_xxx event codes */
    virtual RQEventCode preanalyseAppCommandEvent(int commandCode);
    /** Implemements the pure RQ state machine */
    virtual bool performStateTransition(const RQEventCode& event);
    /** Perform cleanup necessary when entering a new state, e.g. cancelling timers */
    virtual void stateEntered(int state, int oldState, RQEventCode event);
    //@}

    /** @name Processing app commands. Invoked from processAppCommand(). */
    //@{
    virtual void process_OPEN_ACTIVE(RQEventCode& event, RQCommand *RQCommand,
            cMessage *msg);
    virtual void process_OPEN_PASSIVE(RQEventCode& event, RQCommand *RQCommand,
            cMessage *msg);
    virtual void process_SEND(RQEventCode& event, RQCommand *RQCommand,
            cMessage *msg);
    virtual void process_CLOSE(RQEventCode& event, RQCommand *RQCommand,
            cMessage *msg);
    virtual void process_ABORT(RQEventCode& event, RQCommand *RQCommand,
            cMessage *msg);
    virtual void process_STATUS(RQEventCode& event, RQCommand *RQCommand,
            cMessage *msg);
    virtual void process_QUEUE_BYTES_LIMIT(RQEventCode& event,
            RQCommand *RQCommand, cMessage *msg);
    virtual void process_READ_REQUEST(RQEventCode& event, RQCommand *RQCommand,
            cMessage *msg);
    //@}

    /** @name Processing RQ segment arrivals. Invoked from processRQSegment(). */
    //@{
    /**
     * Shortcut to process most common case as fast as possible. Returns false
     * if segment requires normal (slow) route.
     */
    virtual bool tryFastRoute(RQSegment *RQseg);
    /**
     * Process incoming RQ segment. Returns a specific event code (e.g. RQ_E_RCV_SYN)
     * which will drive the state machine.
     */
    virtual RQEventCode process_RCV_SEGMENT(RQSegment *RQseg, L3Address src,
            L3Address dest);
    virtual RQEventCode processSegmentInListen(RQSegment *RQseg, L3Address src,
            L3Address dest);
    virtual RQEventCode processSegmentInSynSent(RQSegment *RQseg, L3Address src,
            L3Address dest);
    virtual RQEventCode processSegment1stThru8th(RQSegment *RQseg);
    virtual RQEventCode processRstInSynReceived(RQSegment *RQseg);
    virtual bool processAckInEstabEtc(RQSegment *RQseg);
    //@}

    /** @name Processing of RQ options. Invoked from readHeaderOptions(). Return value indicates whether the option was valid. */
    //@{
    virtual bool processMSSOption(RQSegment *RQseg,
            const RQOptionMaxSegmentSize& option);
    virtual bool processWSOption(RQSegment *RQseg,
            const RQOptionWindowScale& option);
//    virtual bool processSACKPermittedOption(RQSegment *RQseg, const RQOptionSackPermitted& option);
//    virtual bool processSACKOption(RQSegment *RQseg, const RQOptionSack& option);
    virtual bool processTSOption(RQSegment *RQseg,
    ...
    virtual void process_TIMEOUT_SYN_REXMIT(RQEventCode& event);
    //@}
   ...
   // MOH
    ...
    virtual RQConnection *cloneListeningConnection();

    /** Utility: creates send/receive queues and RQAlgorithm */
    virtual void initConnection(RQOpenCommand *openCmd);
...
//    virtual bool isSegmentAcceptable(RQSegment *RQseg) const;

    /** Utility: send SYN */
    virtual void sendSyn();


    virtual void sendToIPMulticast();
    virtual void  sendToIPMulticastFirstACK();
...
public:
    ...
    virtual void sendToIP(RQSegment *RQseg);

    /**
     * Utility: This factory method gets invoked throughout the RQ model to
     * create a RQSegment. Override it if you need to subclass RQSegment.
     */
    virtual RQSegment *createRQSegment(const char *name);
    ...
    void scheduleTimeout(cMessage *msg, simtime_t timeout) {
        rqMain->scheduleAt(simTime() + timeout, msg);
    }

protected:
    /** Utility: cancel a timer */
    cMessage *cancelEvent(cMessage *msg) {
        return rqMain->cancelEvent(msg);
    }

    /** Utility: send IP packet */
    static void sendToIP(RQSegment *RQseg, L3Address src, L3Address dest);

    /** Utility: sends packet to application */
    virtual void sendToApp(cMessage *msg);

    /** Utility: sends status indication (RQ_I_xxx) to application */
    virtual void sendIndicationToApp(int code, const int id = 0);

    /** Utility: sends RQ_I_ESTABLISHED indication with RQConnectInfo to application */
...
public:
    ...
    static void printSegmentBrief(RQSegment *RQseg);
    /** Utility: returns name of RQ_S_xxx constants */
    static const char *stateName(int state);
    /** Utility: returns name of RQ_E_xxx constants */
    static const char *eventName(int event);
    /** Utility: returns name of RQ_I_xxx constants */
    static const char *indicationName(int code);
    /** Utility: returns name of RQOPTION_xxx constants */
    ...
    /** Utility: returns true when receive queue has enough space for store the RQseg */
    virtual bool hasEnoughSpaceForSegmentInReceiveQueue(RQSegment *RQseg);
    ...
    virtual void updateWndInfo(RQSegment *RQseg, bool doAlways = false);

public:
    /**
     * The "normal" constructor.
     */
    RQConnection(RaptorQ *mod, int appGateIndex, int connId);

    /**
     * Note: this default ctor is NOT used to create live connections, only
     * temporary ones so that RQMain can invoke their segmentArrivalWhileClosed().
     */
    RQConnection();
     ...
     * Destructor.
     */
    virtual ~RQConnection();

    /**
     * This method gets invoked from RQ when a segment arrives which
     * doesn't belong to an existing connection. RQ creates a temporary
     * connection object so that it can call this method, then immediately
     * deletes it.
     */
    virtual void segmentArrivalWhileClosed(RQSegment *RQseg, L3Address src,
            L3Address dest);
    ...
    RQStateVariables *getState() {
        return state;
    }
    RQSendQueue *getSendQueue() {
        return sendQueue;
    }
    // RQSACKRexmitQueue *getRexmitQueue() { return rexmitQueue; }
    RQReceiveQueue *getReceiveQueue() {
        return receiveQueue;
    }
    RQAlgorithm *getRQAlgorithm() {
        return rqAlgorithm;
    }
    RaptorQ *getRQMain() {
        return rqMain;
     ...
     * connection structure must be deleted by the caller (RQ).
     */
    virtual bool processTimer(cMessage *msg);

    /**
     * Process incoming RQ segment. Normally returns true. A return value
     * of false means that the connection structure must be deleted by the
     * caller (RQ).
     */
    virtual bool processRQSegment(RQSegment *RQSeg, L3Address srcAddr,
            L3Address destAddr);
    ...
    /**
     * For SACK RQ. RFC 3517, page 3: "This routine returns whether the given
...
//    virtual bool isLost(uint32 seqNum);
    /**
     * For SACK RQ. RFC 3517, page 3: "This routine traverses the sequence
     ...
     * the RQ sender and the RQ receiver."
     */
//    virtual void setPipe();
    /**
     * For SACK RQ. RFC 3517, page 3: "This routine uses the scoreboard data
...
//    virtual void sendSegmentDuringLossRecoveryPhase(uint32 seqNum);
     ...
     */
    virtual void sendOneNewSegment(bool fullSegmentsOnly,
...
#endif // ifndef __INET_RQCONNECTION_H
