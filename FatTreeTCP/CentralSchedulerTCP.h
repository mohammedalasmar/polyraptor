...
protected:
    ...
    unsigned int numThreeReplicasMultiCastSessions;
    const char * multiUniSourcing;
    unsigned int  numThreeReplicasMultiSourceSessions;
...
protected:
    ...
    void scheduleNewMultiCastSession(std::string  itsSrc, std::vector<std::string>  newDest);

    void getNewThreeSrcRandTMForMultiSourcing(std::string& destNode, std::vector<std::string>& senders );
    void scheduleNewMultiSourcingSession(std::string  dest, std::vector<std::string>  senders);
