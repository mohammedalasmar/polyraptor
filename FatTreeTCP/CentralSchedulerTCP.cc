...
//    stopwatch.setRealTimeLimit(5);
    ...
    numThreeReplicasMultiCastSessions = par("numThreeReplicasMultiCastSessions");

    multiUniSourcing = par("multiUniSourcing");
    numThreeReplicasMultiSourceSessions = par("numThreeReplicasMultiSourceSessions");
...
//
    ...
    if (strcmp(multiUnicasting, "supported") == 0) {
        for (unsigned int i = 1; i <= numThreeReplicasMultiCastSessions ; i++) {
             ...
             scheduleNewMultiCastSession(itsSrc , replicasNodes);
        }
    } else if (strcmp(multiUniSourcing, "supported") == 0) {
        for (unsigned int i = 1; i <= numThreeReplicasMultiSourceSessions ; i++) {
            std::cout << "\n\n\n multi-sourcing Group ID: " << i << std::endl;
            std::vector<std::string> senders;
            getNewThreeSrcRandTMForMultiSourcing(newDest , senders); // tcp multicasting (3 replicas) based on multi Unicasting
            scheduleNewMultiSourcingSession(newDest , senders);
        }
    }else {
...
void CentralSchedulerTCP::scheduleNewMultiCastSession(std::string  itsSrc, std::vector<std::string>  newDest) // three replicas
{
...
void CentralSchedulerTCP::scheduleNewMultiSourcingSession(std::string  dest, std::vector<std::string>  senders) // three replicas
{
...
// random TM for multiUnisourcing (multicast 3 replicas)
...
//            std::cout << "wwwwwwww " << newDestination << " , multicast source node   " << srcNewDestination << "\n";
    ...
    // TODO for this tcp multicasting
//    RecordMat recordMat;
//    recordMat.recordSrc = srcNewDestination;
//    recordMat.recordDest = newDestination;
//    recordMatList.push_back(recordMat);
